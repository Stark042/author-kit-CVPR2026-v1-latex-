...
Let $\mathcal{C}$ denote the set of in-distribution (ID) classes. We quantify class-frequency bias by comparing: (i) the empirical class prior from training data, $p_{\mathrm{tr}}(c)$, and (ii) the normalized predicted-class distribution on evaluation data, $\hat{p}(c)$:
\begin{equation}
    \hat{p}(c)=\frac{N_c}{\sum_{k\in\mathcal{C}}N_k}, \qquad c\in\mathcal{C},
\end{equation}
where $N_c$ is the number of detections assigned to class $c$ above confidence threshold $\tau$.

We then compute a prior-alignment bias score using Kullback--Leibler divergence,
\begin{equation}
    B_{\mathrm{KL}} = \sum_{c\in\mathcal{C}}\hat{p}(c)\log\frac{\hat{p}(c)}{p_{\mathrm{tr}}(c)+\varepsilon},
\end{equation}
and a concentration score,
\begin{equation}
    B_{\mathrm{HHI}} = \sum_{c\in\mathcal{C}} \hat{p}(c)^2,
\end{equation}
where higher $B_{\mathrm{HHI}}$ indicates stronger collapse toward a few dominant classes.

To further capture systematic bias toward frequent classes, we compute rank correlation:
\begin{equation}
    \rho_{\mathrm{freq}} = \mathrm{corr}\!\left(\{\hat{p}(c)\}, \{p_{\mathrm{tr}}(c)\}\right).
\end{equation}

Lower values of $B_{\mathrm{HHI}}$ and $\rho_{\mathrm{freq}}$ indicate improved class balance and reduced reliance on dataset frequency priors.

\vspace{0.5em}
Although OoD-aware fine-tuning improves robustness, it does not explicitly constrain low-level spectral artifacts at inference time. Let the detection score be $s = g(z)$ for feature representation $z$. Under distribution shift, high-frequency perturbations $\delta_{\mathrm{hf}}$ can inflate scores:
\begin{equation}
    \Delta s \approx \nabla_z g(z)^\top \delta_{\mathrm{hf}} > 0
\end{equation}
for non-object regions. This motivates a lightweight post-hoc correction mechanism.
